...
"""
    ...
    def analyze_transcript_from_url(self, json_str):
...
#!/usr/bin/env python
"""
transcript_json.py - Process JSON transcript data provided directly through stdin
"""
...
def main():
    ...
    try:
        ...
        transcript_json = stdin_input  # This is the raw JSON string
        ...
        # Process the JSON directly instead of fetching from URL
        try:
            # We already have the JSON string, no need to fetch it
            report_content = analyzer.analyze_transcript_from_json(transcript_json)
            print(report_content)
            
            # Save the report to markdown file
            with open('all_reports.md', 'w') as f:
                f.write("# transcript_1_report\n\n")
                f.write(report_content)
                f.write("\n\n---\n\n")
            
            # Save the image with unique name
            if os.path.exists('call_analysis.png'):
                os.rename('call_analysis.png', 'transcript_1_analysis.png')
        ...
        results = analyze_md_content(report_content) if 'analyze_md_content' in globals() else None
